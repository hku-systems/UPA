  ...
  extends RDD[T](inputoriginal)
{
  ...
  def reduceDP(f: (T, T) => T) : (RDD[Array[T]],RDD[Array[T]],T) = {
    ...
    val sample_array = sample_count match {
      ...
      case b if b == 1 =>
        val only_array = new Array[T](1)
        only_array(0) = f(result,sample.head.reduce(f))
      ...
      val up_to_index = (sample_count - i).toInt
        ...
        original.sparkContext.parallelize(0 to up_to_index - 1) //(0,1,2,3,4,5,6,7)
          ...
          while (j >= 1) {
            if(j == b_i.value - 1)
              neighnout_o(j - 1) = f(upper_array.value(p), broadcast_sample.value(p + j + 1).reduce(f)) //add back 11, so would be 1 to 10
            else
              neighnout_o(j - 1) = f(broadcast_sample.value(p + j + 1).reduce(f), neighnout_o(j))
            j = j - 1
          }
          neighnout_o
        })
    ...
    val sample_array_advance = sample_advance_count match {
      ...
      case b if b == 1 =>
        var only_array_advance = new Array[T](1)
        only_array_advance(0) = f(aggregatedResult,sample_advance.head.reduce(f))
      ...
      var i = outer_num
        ...
        original.sparkContext.parallelize(0 to up_to_index - 1)
          .map(p => {
            ...
            while (j < b_i.value) {
              if(j == 0)
                neighnout_o(j) = f(broadcast_sample_advance.value(p).reduce(f), broadcast_aggregatedResult.value)
              else
                neighnout_o(j) = f(broadcast_sample_advance.value(p + j).reduce(f), neighnout_o(j-1))
              j = j + 1
            }
            neighnout_o
          })
