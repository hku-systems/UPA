...
object Q51 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val lineitem_input = spark.sparkContext.textFile(inputDir + "/lineitem.tbl*").map(_.split('|')).map(p =>
    ...
    val nation_input = spark.sparkContext.textFile(inputDir + "/nation.tbl*").map(_.split('|')).map(p =>
