...
object TPCH4 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val order_filtered = spark.sparkContext.textFile(inputDir + "/orders"+ args(0) +".ftbl*").map(_.split('|')).map(p =>
      (p(0).trim.toLong, (p(4).trim, p(5).trim)))
      .filter(p => p._2._1 >= "1993-07-01" && p._2._1 < "1993-10-01")

    val lineitem_filtered = spark.sparkContext.textFile(inputDir + "/lineitem"+ args(0) +".ftbl*").map(_.split('|')).map(p =>
